...
pub(crate) fn register_host_globals<'js>(
    ...
    ctx.globals().set(
        "print",
        Func::new(move |_ctx: Ctx<'js>, args: Rest<Value<'js>>| -> rquickjs::Result<()> {
            let parts: Vec<String> = args.0.iter().map(|v| format_js_value(v)).collect();
            let line = parts.join(" ");
            hs.borrow_mut().stdout.push(line);
            Ok(())
        }),
    )?;
    ...
    ctx.globals().set(
        "input",
        Func::new(move |_ctx: Ctx<'js>, _args: Rest<Value<'js>>| -> rquickjs::Result<String> {
            let hs = hs.borrow();
    ...
    ctx.globals().set(
        "read",
        Func::new(move |_ctx: Ctx<'js>, _args: Rest<Value<'js>>| -> rquickjs::Result<Value<'js>> {
    ...
    ctx.globals().set(
        "__webJsTriggerAsync",
        Func::new(move |ctx: Ctx<'js>, args: Rest<Value<'js>>| -> rquickjs::Result<()> {
            let action_str = args
                .0
                .get(0)
    ...
    ctx.globals().set("__webJsPending", Object::new(ctx.clone())?)?;
    ctx.globals().set("__webJsCellResult$", Value::new_undefined(ctx.clone()))?;
    ctx.globals().set("__webJsCellError$", Value::new_undefined(ctx.clone()))?;
