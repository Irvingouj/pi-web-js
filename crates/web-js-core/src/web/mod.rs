...
pub(crate) fn register_web_module<'js>(
    ...
    ctx.globals().set(
        "__webJsLog",
        Func::new(move |_ctx: Ctx<'js>, args: Rest<Value<'js>>| -> rquickjs::Result<String> {
    ...
    ctx.globals().set(
        "__webJsUrlParse",
        Func::new(move |ctx: Ctx<'js>, args: Rest<Value<'js>>| -> rquickjs::Result<Value<'js>> {
            let url_str = args
                .0
                .get(0)
    ...
    ctx.globals().set(
        "__webJsUrlEncode",
        Func::new(move |ctx: Ctx<'js>, args: Rest<Value<'js>>| -> rquickjs::Result<String> {
            let params_val = args.0.get(0).cloned().unwrap_or_else(|| Value::new_undefined(ctx.clone()));
            let json_str = ctx
    ...
    ctx.globals().set(
        ...
        Func::new(move |ctx: Ctx<'js>, _args: Rest<Value<'js>>| -> rquickjs::Result<Value<'js>> {
            ...
            for key_res in global.own_keys::<String>(rquickjs::object::Filter::new().string()) {
                ...
                if type_name == "object" && !value.is_null() {
                        ...
                        if let Some(obj_map) = entry.as_object_mut() {
                            obj_map.insert(
                                ...
                                serde_json::Value::Array(
                                    obj_keys.into_iter().map(serde_json::Value::String).collect(),
                                ),
                            );
                        }
                    }
                    ...
                    if let Some(obj_map) = entry.as_object_mut() {
                        obj_map.insert("value".to_string(), serde_json::Value::String(formatted));
                    }
                }
    ...
    if let Err(e) = ctx.eval::<i32, _>("1+1") {
        ...
        return Err(rquickjs::Error::new_from_js_message("web_module", "eval_test", msg));
    }
    ...
    if let Err(e) = ctx.eval::<(), _>(setup_js) {
        ...
        return Err(rquickjs::Error::new_from_js_message("web_module", "eval", msg));
    }
