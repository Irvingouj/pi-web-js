...
export async function executeMainThreadCommand(command: Command): Promise<AsyncResponse> {
  throwIfAborted();
  const log = logger.child("runner"), finish = log.timer("command_dispatch", { action: command.action, commandId: command.call_id, runId: command.runId });
  ...
  const n = normalizeParams(command.action, command.params), r = await dispatchTool(command.action, n); finish({ ok: r.ok }); return r;
}
