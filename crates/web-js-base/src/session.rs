...
{
    let span = tracing::info_span!("run_cell_async_loop");
    async {
        tracing::info!(code_len = code.len(), "start");
        let mut result = base.run_cell(code, stdin);
        tracing::info!(result = ?result.status(), "run_cell_done");
        ...
        } = &result
        {
            ...
            let actions_str = batch
                ...
                .collect::<Vec<_>>()
                .join(", ");
            tracing::info!(batch_id = %batch_id, batch_size = batch.len(), actions = %actions_str, "batch_created");
                ...
                call_ids.into_iter().zip(responses).enumerate()
            {
                ...
                tracing::info!(call_id, action = %action, "resume_start");
                ...
                tracing::info!(call_id, ok = response.ok, "resume_response");
                let json = serde_json::to_string(&response).unwrap_or_default();
                result = base.resume_cell(call_id, &json);
                tracing::info!(call_id, result = ?result.status(), "resume_done");
            ...
            tracing::info!(batch_id = %batch_id, accumulated_pending = accumulated_pending.len(), "batch_end");
            ...
            tracing::info!(result = ?result.status(), "loop_iteration_end");
            // Loop will check result again — if Pending with new commands, they'll be batched
        }

        tracing::info!(result = ?result.status(), "returning");
        result
