  ...
  try {
    ...
    await popup.evaluate((cells) => {
      ...
      cells.forEach((cellSource: string) => {
        const addButton = document.querySelector('[data-testid="add-cell-button"]') as HTMLElement;
    ...
    while (Date.now() - startTime < maxWaitTime) {
      const allFinished = await popup.evaluate(() => {
        const statuses = document.querySelectorAll('[data-testid="cell-status"]');
        ...
        return {
          ...
          success: Array.from(statuses).filter(s => (s as HTMLElement).textContent?.toLowerCase() === 'success').length,
          error: Array.from(statuses).filter(s => (s as HTMLElement).textContent?.toLowerCase() === 'error').length,
      ...
      console.log(`Progress: ${allFinished.success} success, ${allFinished.error} error, ${allFinished.running} running/idle / ${allFinished.total} total`);
    ...
    const finalResults = await popup.evaluate(() => {
      ...
      return Array.from(cells).map((cell, i) => {
        const status = cell.querySelector('[data-testid="cell-status"]') as HTMLElement;
        const output = cell.querySelector('[data-testid="cell-output"]') as HTMLElement;
        return {
          index: i,
    ...
    console.log("\n=== FINAL RESULTS ===");
    const successCount = finalResults.filter(r => r.status.toLowerCase() === 'success').length;
    const errorCount = finalResults.filter(r => r.status.toLowerCase() === 'error').length;
