...
async function prepareFileUploadTab(harness: {
	fixtureTab: { goto(url: string, opts?: object): Promise<unknown>; bringToFront(): Promise<void> };
}): Promise<void> {
...
test.describe.serial("file-upload-form page.setFiles", () => {
	...
	test("page.setFiles uploads via same-origin URL without passing bytes", async ({
		...
		if (exec.result?.ok) {
			expect(exec.result.value.statusText).toContain("uploaded:photo.jpg:6636");
