...
async function prepareComplexFormTab(harness: {
	fixtureTab: { goto: (url: string, opts?: object) => Promise<unknown>; bringToFront: () => Promise<unknown> };
}): Promise<void> {
...
test.describe.serial("complex-form page APIs", () => {
	...
	test("page.fill sets text on contenteditable element", async ({
		...
		if (exec.result?.ok) {
			...
			expect(exec.result.value.status).toContain("contenteditable:hello editable");
		}
	});
	...
	test("page.submit triggers form submit event", async ({
		harness,
	}) => {
		const exec = await executeCell<
			ContractResult<{ status: string }>
	...
	test("page.select_option on custom ARIA listbox picks option", async ({ harness }) => {
		const exec = await executeCell<
			ContractResult<{ status: string }>
		>(
			harness.sidepanel,
