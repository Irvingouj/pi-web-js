...
> {
	const exec = await executeCell<
		ContractResult<{ count: number; docs: unknown[] }>
	>(
		sidepanel,
		`
...
const AC_USED_APIS = [
	...
	},
	{
		...
		publicName: "fs.writeBase64",
		example:
			'fs.writeBase64({ path: "/tmp/contract-test.txt", data: "SGVsbG8gV29ybGQh" })',
	},
	{
		action: "fs_stat",
		publicName: "fs.stat",
		example: 'fs.stat({ path: "/tmp/contract-test.txt" })',
	},
	{
		action: "fs_hash",
		publicName: "fs.hash",
		example: 'fs.hash({ path: "/tmp/contract-test.txt", algo: "sha256" })',
	},
	{
		action: "page_health",
		publicName: "page.health",
		example: "page.health()",
	},
	{
		action: "page_goto",
		publicName: "page.goto",
...
test.describe
	.serial("T-020: AC-6 documentation contract", () => {
			...
			for (const api of AC_USED_APIS) {
				...
				for (const param of doc.params) {
					...
					if (BANNED_TYPES.has(paramType)) {
						violations.push(
							`${api.publicName} paramsDoc[${param.name}].type="${paramType}"`,
						);
					}
				}
			}
...
`,
		...
		test("page.find / snapshot / snapshot_data examples run unchanged", async ({
			harness,
		}) => {
			await harness.fixtureTab.goto(SIMPLE_FORM_1_URL, {
				waitUntil: "domcontentloaded",
...
`,
			...
			expect(parsed?.ok, `Cell returned error: ${JSON.stringify(parsed)}`).toBe(
				true,
			);
			if (!parsed?.ok) return;
...
`,
		...
		test("page.click / page.fill examples run unchanged", async ({
			harness,
		}) => {
			await harness.fixtureTab.goto(SIMPLE_FORM_1_URL, {
				waitUntil: "domcontentloaded",
			...
			const tabPattern = `${SIMPLE_FORM_1_URL}*`;
			const exec = await executeCell<
				ContractResult<{ fill: unknown; click: unknown }>
			>(
				harness.sidepanel,
				`
var RESULT_PREFIX = "${RESULT_PREFIX}";
...
`,
		...
		test("fs.writeBase64 / fs.stat / fs.hash examples run unchanged", async ({
			harness,
		}) => {
			const exec = await executeCell<
				ContractResult<{ write: unknown; stat: unknown; hash: unknown }>
			>(
				harness.sidepanel,
				`
var RESULT_PREFIX = "${RESULT_PREFIX}";
