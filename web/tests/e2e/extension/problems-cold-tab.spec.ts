...
test.describe.serial("cold tab read/write consistency (AC-4)", () => {
	...
	test("page.health reports missing content script on cold tab", async ({ harness }) => {
	...
	test("mutation on cold tab returns specific error without raw Chrome strings", async ({ harness }) => {
	...
	test("recovery via navigation enables mutations with explicit receipts", async ({ harness }) => {
		// 1. Health on cold tab
		const healthExec = await runAgentCell<ContractResult<{ health: unknown }>>(
			harness.sidepanel,
			[
				"const health = await page.health();",
				"print(RESULT_PREFIX + JSON.stringify({ ok: true, value: { health } }));",
		...
		if (healthExec.result?.ok) {
			const health = healthExec.result.value.health as Record<string, unknown>;
			expect(health.mutationsReady).toBe(false);
			expect(health.contentScript).toBe("missing");
		}
		...
		const healthAfterExec = await runAgentCell<ContractResult<{ health: unknown }>>(
			harness.sidepanel,
			[
		...
		if (healthAfterExec.result?.ok) {
			const health = healthAfterExec.result.value.health as Record<string, unknown>;
		...
		expect(mutationExec.status, `${mutationExec.stderr}\n${mutationExec.stdout}`).toBe("success");
		expect(mutationExec.result?.ok).toBe(true);
		if (mutationExec.result?.ok) {
			const fillResult = mutationExec.result.value.fillResult as Record<string, unknown>;
			const clickResult = mutationExec.result.value.clickResult as Record<string, unknown>;
