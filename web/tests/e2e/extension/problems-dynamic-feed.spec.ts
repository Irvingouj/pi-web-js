...
import { DYNAMIC_FEED_URL, RESULT_PREFIX } from "./lib/constants.ts";
...
test.describe
	.serial("dynamic-feed observation (AC-1)", () => {
		...
		test("T-004: snapshot_data includes articles with images", async ({
			harness,
		}) => {
			const exec = await executeCell<
				ContractResult<{
		...
		test("T-004: image src is absolute and alt is present", async ({
			harness,
		}) => {
			const exec = await executeCell<
				ContractResult<{
