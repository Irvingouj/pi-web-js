...
import { RESULT_PREFIX, MEDIA_DOWNLOAD_URL } from "./lib/constants.ts";
...
test.describe.serial("media-download binary pipeline (AC-3, T-012)", () => {
	...
	test("T-012: full image download and verify pipeline", async ({ harness }) => {
		...
		if (exec.result?.ok) {
			...
			expect(exec.result.value.fileHash).toBe(
				exec.result.value.expectedHash,
			);
		}
	});

	test("T-012: text fetch still works alongside binary", async ({ harness }) => {
		const exec = await executeCell<
			ContractResult<{
				bodyEncoding: string;
				body: string;
