...
(async () => {
  ...
  const t1 = await runCode('function F() { return undefined; } new F()');
  console.log('new F():', JSON.stringify(t1));

  const t2 = await runCode('function G() { return {}; } new G()');
  console.log('new G():', JSON.stringify(t2));

  const t3 = await runCode('var e = new Error("boom"); e');
  console.log('new Error:', JSON.stringify(t3));
