...
import { syntaxHighlighting, indentOnInput, defaultHighlightStyle, foldGutter, bracketMatching } from '@codemirror/language';
...
const jsBuiltins = [
  ...
  'String', 'Number', 'Boolean', 'Date', 'Math.random', 'Math.floor', 'Math.ceil',
  'Math.max', 'Math.min', 'Math.abs', 'Math.sqrt', 'Math.PI',
...
const notebookGlobals = [
  ...
  'page.scroll', 'page.scrollTo', 'page.url', 'page.title', 'page.screenshot',
  'page.goto', 'page.back', 'page.forward', 'page.reload', 'page.wait',
  'page.tabs', 'page.switch', 'page.newTab', 'page.close', 'page.activeTab',
  ...
  'tab.goto', 'tab.scroll', 'tab.scrollTo',
...
function jsCompletions(context: CompletionContext): CompletionResult | null {
  ...
  const options = [
    ...
    ...notebookGlobals.map(k => ({ label: k, type: 'function', detail: 'notebook API' })),
  ];
