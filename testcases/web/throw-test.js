...
(async () => {
  ...
  const t1 = await runCode('throw "boom"');
  console.log('throw string:', JSON.stringify(t1));

  const t2 = await runCode('throw 1');
  console.log('throw number:', JSON.stringify(t2));

  const t3 = await runCode('throw new Error("boom")');
  console.log('throw Error:', JSON.stringify(t3));
